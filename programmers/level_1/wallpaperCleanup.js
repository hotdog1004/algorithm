 ...
 * 1 ≤ wallpaper의 길이 ≤ 50
 * 1 ≤ wallpaper[i]의 길이 ≤ 50 (wallpaper의 모든 원소의 길이는 동일합니다.)
...
function solution(wallpaper){
  // '#'인 값의 좌표를 각 배열에 담아 시작점과 끝점을 구한다.
  // 끝점일 경우 파일이 포함되어야하니 1씩 더하기
  let lux, luy, rdx, rdy;
  let xArr = [];
  let yArr = [];
  let line = '';
  ...
  [lux, luy] = [Math.min(...xArr), Math.min(...yArr)];
  [rdx, rdy] = [Math.max(...xArr)+1, Math.max(...yArr)+1]
