 ...
 * 1 ≤ keymap의 길이 ≤ 100. keymap[i]는 i + 1번 키를 눌렀을 때 순서대로 바뀌는 문자를 의미합니다.
 * keymap의 원소의 길이는 서로 다를 수 있습니다. keymap의 원소는 알파벳 대문자로만 이루어져 있습니다.
 * 1 ≤ targets의 길이 ≤ 100. 1 ≤ targets의 원소의 길이 ≤ 100. targets의 원소는 알파벳 대문자로만 이루어져 있습니다.
 */

function solution(keymap,targets) {
  let answer = targets.map(str=>{
  let arr = new Array(str.length).fill(101);
    for(let i = 0; i < str.length; i++){
        for(let key of keymap){
            if(key.indexOf(str[i]) !== -1){
                arr[i] = Math.min(arr[i], key.indexOf(str[i])+1);
                } 
        }
    }
    return arr;
